...
```{r}
seurat_obj <- readRDS("data/kang2018.rds")

```

#### Discussion: The Seurat Object in R { - .rational}

Lets take a look at the seurat object we have just created in R, `seurat_obj`
...
The `assays` slot in `seurat_obj` can be accessed with `seurat_obj@assays`.

The `RNA` assay can be accessed from this with `seurat_obj@assays$RNA`. 

We often want to access assays, so Seurat nicely gives us a shortcut `seurat_obj$RNA`. You may sometimes see an alternative notation `seurat_obj[["RNA"]]`.
...
Use `str` to look at the structure of the Seurat object `seurat_obj`.
...
```{r}
GetAssayData(seurat_obj)[1:15,1:2]
```


But the raw counts data is accessible too.
```{r}
GetAssayData(seurat_obj, slot='counts')[1:15,1:2]
```
